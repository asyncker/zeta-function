...
a = +0 or -0
```
f(z, u, t)⁺⁻ = 1 ± u / (t * z^2) * (exp(u / (z * t)) / (exp(u / (z * t)) - 1))
```
...
```
sin(z * pi/2) * 2i = (1 - (-1)^(-z/2)) - (1 - (-1)^(z/2))
sin(z * pi/2) * 2 = ((-1)^(z/2) - (-1)^(-z/2)) * -1i
```
...
◇ means there is no function value at this point. All functions except the equal sign (=) are prohibited, and ◇ is not equal to anything except itself. You can write x = ◇
```
relumaxU(z) = z >= +0 ? z : ◇
reluminU(z) = z <= -0 ? z : ◇
```
...
```
BL(z) = (z.x - z.w) + (z.y + z.z) * i
BR(z) = (z.x + z.w) + (z.y - z.z) * i
BW(z, w) = 1/2 * (w.x + z.x, z.y + w.y, z.y - w.y, w.x - z.x)
bizeta(z) = BW(ζ(BL(z)), ζ(BR(z)))
```
...
ptlim(10^n + 7) = {0, 1} | {1, 2, 4, 8, 9, 24, 60, 110, 134, 222, 412, 700, 999...}
ptlim(10^(2n) - 10^(1n) + 1) = 0 (n > 8) | {2, 4, 6, 8}
ptlim(12 * (2^n - 1) - 1) = {0, 1} | {1, 3, 4, 7, 9, 12, 15, 16, 25, 37, 40, 43, 49, 72, 75, 124, 180, 196, 208, 312, 480, 568, 1113, 1729, 2244, 3129...}

so many primes x^n * k - u:
ptlim(2^n * 24 - 25) = {0, 1} | {1, 2, 3, 4, 5, 6, 9, 10, 13, 14, 15, 18, 20, 26, 28, 30, 32, 33, 38, 55, 60, 63, 66, 79, 93, 96, 145, 154, 172, 193, 238, 264, 346, 391, 401, 411, 432, 453, 482, 495, 534, 545, 581, 712, 724, 762, 926, 1004, 1073, 1396, 1616, 1649, 1712, 2755, 3036, 3128, 3770, 3819...}

ptlim(2^n * 21 - 5) = {0, 1} | {1, 2, 3, 4, 7, 10, 11, 12, 13, 20, 21, 24, 25, 26, 31, 35, 45, 52, 53, 55, 63, 65, 67, 70, 76, 77, 81, 107, 113, 122, 135, 153, 157, 226, 238, 262, 284, 374, 466, 480, 538, 697, 710, 741, 1255, 1661, 1692, 1906, 1936, 2091, 2180, 3384, 3406, 3531, 3915...}
```
