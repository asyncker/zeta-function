...
Нули zsinh и zcosh чередуется при t<1/2 как только t=1/2 чередование нарушается. arg(ζ(it)), её производная ~= -ln(t/2π) и логарифм подавляет флуктуации при больших t
...
zeta сложно сделать через quaternion, но легко через bicomplex в 4d, используя идемпотентность. Отобразить zeta в гильбертовом пространстве/эрмитовы операторы является задачей где я копаю. Ещё нужны мультипликативные интегралы и производны не-ньютоновские исчисления. И многозначные функции arcgamma
...
+∞ > ω > 10^1000000
+0 < ε < 10^-1000000
ω, ε отображают маштаб (ω^-7 * 6.246) вот https://asyncker.github.io/fractals/render/scale_plane_algebra.png
...
Можно задать примерные значения ε ~= 0.001 и ω ~= 1000, прикладной смысл:
ln(ε) = -ln(ω)
ln(ω) = -ln(ε)
ln(ω^ε) = -ε * ln(ε)
ε * ω = 1
